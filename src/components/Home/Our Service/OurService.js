const OurService = () => {
  return (
    <div
      className={"bg-[#F3F3F3] mt-[13rem] pt-[8.25rem] px-[10rem] pb-[8.56rem]"}
    >
      <div className={" text-center space-y-[2.5rem]"}>
        <h1 className={"text-[1.2125rem] font-medium tracking-[0.18188rem]"}>
          OUR SERVICES
        </h1>
        <h3 className={"text-[2.4875rem] leading-[2.9375rem] font-semibold"}>
          We Are Committed To Provide Better Service
        </h3>
      </div>
      <div className={"mt-[3.8rem]"}>
        <div className={"flex items-center justify-center gap-[3.13rem]"}>
          <div
            className={"bg-white w-fit p-[3.75rem] rounded-[1.875rem]"}
            style={{ boxShadow: "4px 10px 30px 0px rgba(0, 0, 0, 0.03)" }}
          >
            {/*  svg logo*/}
            <div
              className={"bg-[#4361EE] w-fit p-[1.57rem] rounded-full mx-auto"}
            >
              <svg
                xmlns="http://www.w3.org/2000/svg"
                width="42"
                height="42"
                viewBox="0 0 42 42"
                fill="none"
              >
                <path
                  d="M39.6332 32.9763L32.8763 26.2193L27.6789 24.072C29.4095 21.6111 30.3366 18.6751 30.3333 15.6666C30.3333 7.5794 23.7539 1 15.6666 1C7.5794 1 1 7.5794 1 15.6666C1 23.7539 7.5794 30.3333 15.6666 30.3333C18.7013 30.3367 21.6615 29.3936 24.1349 27.6352L26.2765 32.8185L33.0333 39.5758C33.4666 40.0092 33.981 40.353 34.5472 40.5875C35.1134 40.822 35.7203 40.9428 36.3331 40.9428C36.9459 40.9428 37.5528 40.8221 38.119 40.5876C38.6852 40.3531 39.1997 40.0094 39.633 39.576C40.0664 39.1427 40.4102 38.6283 40.6447 38.0621C40.8792 37.4959 41 36.889 41 36.2762C41 35.6633 40.8793 35.0565 40.6448 34.4903C40.4103 33.9241 40.0666 33.4096 39.6332 32.9763ZM3.66666 15.6666C3.66666 9.04998 9.04998 3.66666 15.6666 3.66666C22.2833 3.66666 27.6666 9.04998 27.6666 15.6666C27.6666 22.2833 22.2833 27.6666 15.6666 27.6666C9.04998 27.6666 3.66666 22.2833 3.66666 15.6666ZM37.7475 37.6902C37.3721 38.0648 36.8635 38.2751 36.3333 38.2751C35.803 38.2751 35.2944 38.0648 34.919 37.6902L28.538 31.3093L26.5469 26.4897L31.3666 28.4809L37.7477 34.8618C38.1221 35.2372 38.3324 35.7458 38.3324 36.2761C38.3324 36.8063 38.122 37.3149 37.7475 37.6902Z"
                  fill="white"
                  stroke="white"
                  strokeWidth="0.6"
                />
              </svg>
            </div>
            <div className={"text-center mt-[1.8rem]"}>
              <h1 className={"text-[1.2125rem] font-medium text-[#2B2B2B]"}>
                Enhance Search Option
              </h1>
              <p
                className={
                  "mt-[1.25rem] leading-[1.625rem] text-[1rem] text-[#808080] font-normal"
                }
              >
                We are provide you the <br /> best search filtering.
              </p>
            </div>
          </div>{" "}
          <div
            className={"bg-white w-fit p-[3.75rem] rounded-[1.875rem]"}
            style={{ boxShadow: "4px 10px 30px 0px rgba(0, 0, 0, 0.03)" }}
          >
            {/*  svg logo*/}
            <div
              className={"bg-[#4361EE] w-fit p-[1.57rem] rounded-full mx-auto"}
            >
              <svg
                xmlns="http://www.w3.org/2000/svg"
                width="42"
                height="42"
                viewBox="0 0 42 42"
                fill="none"
              >
                <path
                  d="M39.6332 32.9763L32.8763 26.2193L27.6789 24.072C29.4095 21.6111 30.3366 18.6751 30.3333 15.6666C30.3333 7.5794 23.7539 1 15.6666 1C7.5794 1 1 7.5794 1 15.6666C1 23.7539 7.5794 30.3333 15.6666 30.3333C18.7013 30.3367 21.6615 29.3936 24.1349 27.6352L26.2765 32.8185L33.0333 39.5758C33.4666 40.0092 33.981 40.353 34.5472 40.5875C35.1134 40.822 35.7203 40.9428 36.3331 40.9428C36.9459 40.9428 37.5528 40.8221 38.119 40.5876C38.6852 40.3531 39.1997 40.0094 39.633 39.576C40.0664 39.1427 40.4102 38.6283 40.6447 38.0621C40.8792 37.4959 41 36.889 41 36.2762C41 35.6633 40.8793 35.0565 40.6448 34.4903C40.4103 33.9241 40.0666 33.4096 39.6332 32.9763ZM3.66666 15.6666C3.66666 9.04998 9.04998 3.66666 15.6666 3.66666C22.2833 3.66666 27.6666 9.04998 27.6666 15.6666C27.6666 22.2833 22.2833 27.6666 15.6666 27.6666C9.04998 27.6666 3.66666 22.2833 3.66666 15.6666ZM37.7475 37.6902C37.3721 38.0648 36.8635 38.2751 36.3333 38.2751C35.803 38.2751 35.2944 38.0648 34.919 37.6902L28.538 31.3093L26.5469 26.4897L31.3666 28.4809L37.7477 34.8618C38.1221 35.2372 38.3324 35.7458 38.3324 36.2761C38.3324 36.8063 38.122 37.3149 37.7475 37.6902Z"
                  fill="white"
                  stroke="white"
                  strokeWidth="0.6"
                />
              </svg>
            </div>
            <div className={"text-center mt-[1.8rem]"}>
              <h1 className={"text-[1.2125rem] font-medium text-[#2B2B2B]"}>
                Enhance Search Option
              </h1>
              <p
                className={
                  "mt-[1.25rem] leading-[1.625rem] text-[1rem] text-[#808080] font-normal"
                }
              >
                We are provide you the <br /> best search filtering.
              </p>
            </div>
          </div>{" "}
          <div
            className={"bg-white w-fit p-[3.75rem] rounded-[1.875rem]"}
            style={{ boxShadow: "4px 10px 30px 0px rgba(0, 0, 0, 0.03)" }}
          >
            {/*  svg logo*/}
            <div
              className={"bg-[#4361EE] w-fit p-[1.57rem] rounded-full mx-auto"}
            >
              <svg
                xmlns="http://www.w3.org/2000/svg"
                width="42"
                height="42"
                viewBox="0 0 42 42"
                fill="none"
              >
                <path
                  d="M39.6332 32.9763L32.8763 26.2193L27.6789 24.072C29.4095 21.6111 30.3366 18.6751 30.3333 15.6666C30.3333 7.5794 23.7539 1 15.6666 1C7.5794 1 1 7.5794 1 15.6666C1 23.7539 7.5794 30.3333 15.6666 30.3333C18.7013 30.3367 21.6615 29.3936 24.1349 27.6352L26.2765 32.8185L33.0333 39.5758C33.4666 40.0092 33.981 40.353 34.5472 40.5875C35.1134 40.822 35.7203 40.9428 36.3331 40.9428C36.9459 40.9428 37.5528 40.8221 38.119 40.5876C38.6852 40.3531 39.1997 40.0094 39.633 39.576C40.0664 39.1427 40.4102 38.6283 40.6447 38.0621C40.8792 37.4959 41 36.889 41 36.2762C41 35.6633 40.8793 35.0565 40.6448 34.4903C40.4103 33.9241 40.0666 33.4096 39.6332 32.9763ZM3.66666 15.6666C3.66666 9.04998 9.04998 3.66666 15.6666 3.66666C22.2833 3.66666 27.6666 9.04998 27.6666 15.6666C27.6666 22.2833 22.2833 27.6666 15.6666 27.6666C9.04998 27.6666 3.66666 22.2833 3.66666 15.6666ZM37.7475 37.6902C37.3721 38.0648 36.8635 38.2751 36.3333 38.2751C35.803 38.2751 35.2944 38.0648 34.919 37.6902L28.538 31.3093L26.5469 26.4897L31.3666 28.4809L37.7477 34.8618C38.1221 35.2372 38.3324 35.7458 38.3324 36.2761C38.3324 36.8063 38.122 37.3149 37.7475 37.6902Z"
                  fill="white"
                  stroke="white"
                  strokeWidth="0.6"
                />
              </svg>
            </div>
            <div className={"text-center mt-[1.8rem]"}>
              <h1 className={"text-[1.2125rem] font-medium text-[#2B2B2B]"}>
                Enhance Search Option
              </h1>
              <p
                className={
                  "mt-[1.25rem] leading-[1.625rem] text-[1rem] text-[#808080] font-normal"
                }
              >
                We are provide you the <br /> best search filtering.
              </p>
            </div>
          </div>{" "}
        </div>
      </div>
    </div>
  );
};

export default OurService;
